...
export async function GET(request: NextRequest) {
  ...
  try {
    const url = `${RAJAONGKIR_V2_BASE}/destination/international-destination?search=${encodeURIComponent(search)}&limit=10&offset=0`;
    ...
    if (!response.ok) {
      ...
      console.error("❌ RajaOngkir international destination search error:", response.status, errorText);
    ...
    console.error("❌ RajaOngkir international destination search failed:", error);
